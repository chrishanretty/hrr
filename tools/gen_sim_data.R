...
ind <- merge(ind, res, by = "area", all = TRUE)

toydata <- as.data.frame(ind)
toyps <- as.data.frame(ps)

###
if (FALSE) {

    usethis::use_data(toydata, overwrite = TRUE)
    usethis::use_data(toyps, overwrite = TRUE)
}

### save(ps, ind, file = "toy_data.rda")
